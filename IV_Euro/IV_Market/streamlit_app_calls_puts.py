#!/usr/bin/env python3
"""Streamlit app to visualize call and put implied volatility surfaces side-by-side."""
...
st.set_page_config(page_title="IV Surfaces (Calls & Puts)", layout="wide")
st.title("3D Implied Volatility Surfaces (Calls & Puts)")
st.write(
    "Download monthly option data via `yfinance`, filter strikes around the underlying spot, "
    "and visualize interpolated call and put implied volatility surfaces in tandem."
)
...
def plot_surface(surface: pd.DataFrame, spot: float, title_suffix: str) -> go.Figure:
    ...
    fig.update_layout(
        title=f"{title_suffix} Implied Volatility Surface (S₀ ± window)",
        scene=dict(
    ...
    if not ticker_input:
        ...
        try:
            ...
            with col_calls:
                ...
                fig_calls = plot_surface(surface_calls, spot, "Call")
                st.plotly_chart(fig_calls, use_container_width=True)
            with col_puts:
                st.subheader("Put IV Surface")
                fig_puts = plot_surface(surface_puts, spot, "Put")
                st.plotly_chart(fig_puts, use_container_width=True)
        except Exception as exc:  # noqa: BLE001
            st.error(f"Failed to download or process data: {exc}")
else:
    st.info("Configure parameters in the sidebar and click **Fetch & Plot** to build both surfaces.")
